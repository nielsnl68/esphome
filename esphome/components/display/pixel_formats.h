...
#define EXPORT_SRC_PIXEL_FORMAT(MACRO, IGNORE_MACRO, ...) \
  ...
  MACRO(RGB666_LE, ##__VA_ARGS__); \
  MACRO(RGB666, ##__VA_ARGS__); \
...
template<PixelFormat FF, uint8_t RR, uint8_t GG, uint8_t BB, uint8_t AA, uint8_t WW, uint8_t NN,
         uint8_t PP = 1, bool CCOLOR_KEY = 0>
struct PixelDetails {
  static const PixelFormat FORMAT = FF;
  static const uint8_t R = RR;
  static const uint8_t G = GG;
  static const uint8_t B = BB;
  static const uint8_t A = AA;
  static const uint8_t W = WW;
  static const uint8_t BYTES = NN;
  static const uint8_t PACKED_PIXELS = PP;
...
typedef PixelRGB565_Endiness<PixelFormat::RGB565, true> PixelRGB565_BE;
