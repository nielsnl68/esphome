...
uint32_t DisplayDriver::get_buffer_length_() {
  result = this->width_ * this->height_;
  switch (this->buffer_bitness_) {
    case COLOR_BITNESS_1:
      return result / 8;
    case COLOR_BITNESS_2:
    case COLOR_BITNESS_2_GRAY:
      return result / 4;
    case COLOR_BITNESS_4:
    case COLOR_BITNESS_4_GRAY:
      return result / 2;
...
void void setup_buffer() {
  if (this->buffer_ != nullptr) {
    return;
  }
  this->buffer_depth_ = this->display_bitness_;
  switch (this->buffer_depth_) {
    case COLOR_BITNESS_666:
    case COLOR_BITNESS_888:
      ...
      this->buffer_depth_ = COLOR_BITNESS_565;
    case COLOR_BITNESS_565:
      this->init_internal_(this->get_buffer_length_());
      if (this->buffer_ != nullptr) {
        return;
      ...
      if (this->palette_ = nullptr) {
        this->buffer_depth_ = COLOR_BITNESS_332;
      } else {
        this->buffer_depth_ = COLOR_BITNESS_8_INDEX;
      }
    default:
      this->init_internal_(this->get_buffer_length_());
      if (this->buffer_ == nullptr) {
...
void DisplayBuffer::allocate_buffer_(uint32_t buffer_length) {
  ExternalRAMAllocator<uint8_t> allocator(ExternalRAMAllocator<uint8_t>::ALLOW_FAILURE);
  this->buffer_ = allocator.allocate(buffer_length);
}

void dunp_bridness(std::string title, Display::ColorBitness bridness) {
  char *depth;
  switch (bridness) {
    case COLOR_BITNESS_1:
      depth = "Monochrome";
    ...
    case COLOR_BITNESS_4:
      ...
      break;
    case COLOR_BITNESS_8:
      depth = "256 colors/grayscale";
      break;
    case COLOR_BITNESS_8I:
      depth = "8bit Indexed";
      break;
    case COLOR_BITNESS_565:
  ...
  ESP_LOGCONFIG(TAG, title, c_str(), depth);
}
...
void DisplayDriver::fill(Color color) {
  this->setup_buffer();
  uint8_t bytes_per_pixel = (uint8_t) this->buffer_depth_ >> 16;
  size_t count = this->get_buffer_length_();

  uint32_t new_color = display::ColorUtil::from_color(color, this->buffer_depth_, this->palette_);

  while (count--) {
    memcpy(buf, val, bytes_per_pixel);
    buf += bytes_per_pixel;
  }
}
...
void HOT DisplayDriver::buffer_pixel_at(int x, int y, Color color) {
  ...
  uint16_t new_color = display::ColorUtil::from_color(color, this->buffer_depth_, this->palette_);
  bool updated = false;
  this->setup_buffer();
...
void DisplayDriver::display_buffer() {
  ...
  uint8_t bytes_per_pixel = (uint8_t) this->buffer_depth_ >> 16;
  uint8_t devider = this->buffer_depth_ >> 24;
  ...
  ESP_LOGV(TAG,
           ...
           this->x_low_, this->y_low_, this->x_high_, this->y_high_, w, h, this->buffer_depth_, this->display_depth_,
           sw_time, mw_time);
  auto now = millis();
  ...
  if (this->buffer_depth_ == this->display_depth_ && sw_time < mw_time) {
    ...
    while (rem-- != 0) {
      ...
      switch (this->buffer_depth_) {
        case COLOR_BITNESS_332:
          color = (display::ColorUtil::rgb332_to_color(this->buffer_[pos++]));
          break;
        case COLOR_BITNESS_8I:
          color =  display::ColorUtil::index8_to_color_palette888(this->buffer_[pos++], this->palette_));
          break;
...
void DisplayDriver::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr,
  ...
  this->bus_->enable();
  this->bus_->set_addr_window_(x_start, y_start, x_start + w - 1, y_start + h - 1);
...
boolean DisplayDriver::set_addr_window(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
...
void displayInterface::send_command(uint8_t command, const uint8_t *data, uint8_t len, boolean enable) {
  if (enable || this->always_enable_) {
    this->enable();
  }
...
void displayInterface::send_data(const uint8_t *data, uint8_t len, boolean enable) {
  boolean old_state = this->if (enable || this->always_enable_) { this->enable(); }
  this->data(data, len);
  if (enable || this->always_enable_) {
    this->disable();
  }
...
void displayInterface::data(uint8_t *value, uint16_t len) {
  this->start_data();
  if (len == 1) {
    this->write_byte(value);
  } else {
    this->write_bytes(value, len);
  }
  this->end_data();
}
...
void SPI_Interface::enable() {
  if (this->enabled_ == 0) {
    spi::SPIDevice::enable();
  }
  this->enabled_++;
}
void SPI_Interface::disable() {
  this->enabled_--;
  if (this->enabled_ == 0) {
    spi::SPIDevice::disable();
    this->enabled_ = 0;
  }
}

void SPI_Interface::write_byte(uint8_t data) { spi::SPIDevice::write_byte(data); }
void SPI_Interface::write_array(uint8_t *data, int16_t len) { spi::SPIDevice::write_array(data, len); };
